...
for ((i = 1; i <= Nreps; i++)); do
	...
	for ((j = 1; j <= 4; j++)); do
		...
		for ((k = TamIni; k < TamFin; k += TamPaso)); do
			...
			time_serie=$(./pescalar_serie $k |tail -n 1|awk '{print $2}')
...
cat pescalar_media.dat |head -n 10 >> pescalar_media_1hilo.dat
cat pescalar_media.dat |head -n 20 |tail -n 10 >> pescalar_media_2hilos.dat
cat pescalar_media.dat |head -n 30 |tail -n 10 >> pescalar_media_3hilos.dat
cat pescalar_media.dat |tail -n 10 >> pescalar_media_4hilos.dat
...
cat pescalar_aceleracion.dat |head -n 10 > pescalar_aceleracion_1hilo.dat
cat pescalar_aceleracion.dat |head -n 20 |tail -n 10 > pescalar_aceleracion_2hilos.dat
cat pescalar_aceleracion.dat |head -n 30 |tail -n 10 > pescalar_aceleracion_3hilos.dat
cat pescalar_aceleracion.dat |tail -n 10 > pescalar_aceleracion_4hilos.dat
...
gnuplot << END_GNUPLOT
	...
	set term png
	set output "$fPNG2"
	plot "pescalar_aceleracion_1hilo.dat" using 2:3 with lines lw 2 title "aceleracion 1 hilo", \
		 "pescalar_aceleracion_2hilos.dat" using 2:3 with lines lw 2 title "aceleracion 2 hilos", \
		 "pescalar_aceleracion_3hilos.dat" using 2:3 with lines lw 2 title "aceleracion 3 hilos", \
		 "pescalar_aceleracion_4hilos.dat" using 2:3 with lines lw 2 title "aceleracion 4 hilos"
	replot
	quit
END_GNUPLOT